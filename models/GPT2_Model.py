...
class GPT2(pl.LightningModule):
    ...
    def configure_optimizers(self, train_len=None):
        ...
        if self.hparams.use_lr_scheduling:
            ...
            total_num_steps = (
                                          steps_per_epoch * self.hparams.num_train_epochs) * self.hparams.num_files * schedule_scale_factor

            print(f'total number of steps : {total_num_steps}')
            scheduler = WarmupDecayLR(optimizer, total_num_steps=total_num_steps,
                                      warmup_max_lr=self.hparams.learning_rate,
                                      warmup_num_steps=int(total_num_steps * 0.1))
    ...
    def train_dataloader(self):
        if self.hparams.mode == 'pretrain_brute':
            train_dataset = Pretrain_Chunks(dataset_name=self.dataset_lst[self.dataset_index], tokenizer=self.tokenizer,
                                            input_length=self.hparams.max_input_length,
                                            output_length=self.hparams.max_output_length, args=self.hparams)
        else:
        ...
        if self.hparams.method == 'mixreview':
            ...
            pretrain_dataset = self.get_dataset(tokenizer=self.tokenizer, type_path="pretrain", args=self.hparams,
                                                length=mix_len)
    ...
    def val_dataloader(self):
        validation_dataset_unchanged = self.get_dataset(tokenizer=self.tokenizer, type_path="validation",
                                                        args=self.hparams, lama_type='unchanged')
        validation_dataset_changed = self.get_dataset(tokenizer=self.tokenizer, type_path="validation",
                                                      args=self.hparams, lama_type='changed')
        return [DataLoader(validation_dataset_unchanged, batch_size=self.hparams.eval_batch_size,
                           num_workers=self.hparams.num_workers, shuffle=False),
                DataLoader(validation_dataset_changed, batch_size=self.hparams.eval_batch_size,
                           num_workers=self.hparams.num_workers, shuffle=False),
                ]
